    ...
    analyzePhase({node, moduleDoc}){
      switch (node.kind) {
        case ts.SyntaxKind.ClassDeclaration:
          ...
          const componentDecorator = node?.decorators?.find(findDecorator('Component'))?.expression;
          ...
          const eventFields = node?.members
            ?.filter(member => member?.decorators?.find(findDecorator('Event')))
            ?.map(member => {
              const eventDecorator = member?.decorators?.find(findDecorator('Event'));
           ...
           */
          node?.members
            ?.filter(member => member?.decorators?.find(findDecorator('Prop')))
            ?.forEach(property => {
